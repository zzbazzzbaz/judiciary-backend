      (function () {
        ...
        function ensureApiClient() {
          ...
          if (!window.AdminApi || !window.AdminApi.createClient) {
            throw new Error("API 客户端未加载，请确认已引入 ./api/api-client.js");
          }
          apiClient = window.AdminApi.createClient(state.apiBase);
          return apiClient;
        }

        async function apiRequest(path, options) {
          return ensureApiClient().request(path, options);
        }
              ...
              if (poly && poly.getPath) {
                try {
                  ...
                  var html =
                    ...
                    "<div style='color:#666;font-size:12px;'>" +
                    "所属区域：" + escapeHtml(grid.region || "-") + "<br>" +
                    "负责人：" + escapeHtml(mgr) + "<br>" +
                    "调解员数量：" + escapeHtml(mediators) + "</div></div>";
                  state.infoWin.setContent(html);
                  state.infoWin.setPosition(b.getCenter());
                  state.infoWin.open();
                } catch (e) {}
              }
            });
            ...
            li.addEventListener("click", function () {
              ...
              if (isFinite(lng) && isFinite(lat) && state.map) {
                try {
                  ...
                  state.map.panTo ? state.map.panTo(pos) : state.map.setCenter(pos);
                  // 显示气泡框
                  var html =
                    "<div style='min-width:240px;line-height:1.6;'>" +
                    "<div style='font-weight:600;margin-bottom:6px;'>" + escapeHtml(task.code || "") + "</div>" +
                    "<div style='color:#666;font-size:12px;'>" +
                    "状态：" + escapeHtml(task.status || "-") + "<br>" +
                    "当事人：" + escapeHtml(task.party_name || "-") + "<br>" +
                    "地址：" + escapeHtml(task.report_address || "-") + "</div></div>";
                  state.infoWin.setContent(html);
                  state.infoWin.setPosition(pos);
                  state.infoWin.open();
                } catch (e) {}
              }
            });
